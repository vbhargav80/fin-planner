...
export const ExpensesManager: React.FC<Props> = ({ categories, dispatch, isAdminMode }) => {
    ...
    const getVisibleCategoryTotal = (cat: ExpenseCategory) => {
        return cat.items.reduce((sum, item) => {
            // Correct Logic: If hidden, don't count. If visible (unhidden), count it.
                            ...
                            {(groupedItems[activeSubGroup] || []).map(item => {
                                ...
                                return (
                                    <InputGroup
                                        key={item.id}
                                        id={item.id}
                                        // FIXED: Label is just text/span now
                                        label={
                                            <span className={item.isHidden ? 'text-red-500' : ''}>
                                                {item.name}
                                            </span>
                                        }
                                        // FIXED: Button moved to labelAction prop
                                        labelAction={
                                            isAdminMode ? (
                                                <button
                                                    ...
                                                    title={item.isHidden ? "Item is hidden (Click to unhide)" : "Item is visible (Click to hide)"}
                                                >
                                                    {item.isHidden
